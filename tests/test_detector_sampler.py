...
def test_dynamic_circuit_rejection():
    ...
    with pytest.raises(NotImplementedError, match="to_stim only supports static circuits"):
        circuit.to_stim()
        
    with pytest.raises(NotImplementedError, match="Detector sampling is not supported for dynamic circuits"):
